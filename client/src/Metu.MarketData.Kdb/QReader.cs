...
{
    /// <summary>
    /// Provides deserialization functionality from KDB Interprocess Communication Protocol.
    /// <para>QReader is not thread-safe.</para>
    /// </summary>
    public sealed class QReader
    {
        ...
        public QMessage Read(bool raw = false)
        {
            ...
            _reader = new BinaryReader(data, endianess);
        ...
        private byte[] ReadFully(int length)
        {
            ...
            while ((chunk = _stream.Read(buffer, read, Math.Min(_maxReadingChunk, length - read))) > 0)
            {
                read += chunk;
            ...
            var uncompressedSize = -8 +
                                   (int)
                                       (endianess == Endianess.BigEndian
                                           ? BigEndianByteConverter(compressed, 0, 4)
                                           : LittleEndianByteConverter(compressed, 0, 4));
